...
async function appendSnapshot(db, snap) {
  const res = await db.prepare(
    `INSERT INTO snapshots (source, fetched_at, observed_at_min, observed_at_max, data)
       VALUES (?1, ?2, ?3, ?4, ?5)`
  ).bind(snap.source, snap.fetched_at, snap.observed_at_min, snap.observed_at_max, JSON.stringify(snap.data)).run();
...
async function appendCollation(db, c) {
  const res = await db.prepare(
    `INSERT INTO collations (collated_at, observed_at_min, observed_at_max, sources)
       VALUES (?1, ?2, ?3, ?4)`
  ).bind(c.collated_at, c.observed_at_min, c.observed_at_max, JSON.stringify(c.sources)).run();
  return Number(res.meta.last_row_id);
}
__name(appendCollation, "appendCollation");
...
__name(latestCollation, "latestCollation");
async function appendLatent(db, l) {
  const res = await db.prepare(
    `INSERT INTO latents (ts, name, value, collation_id, inputs, confidence)
       VALUES (?1, ?2, ?3, ?4, ?5, ?6)`
  ).bind(l.ts, l.name, l.value, l.collation_id, JSON.stringify(l.inputs), l.confidence ?? null).run();
  return Number(res.meta.last_row_id);
}
__name(appendLatent, "appendLatent");
...
__name(rowToLatent, "rowToLatent");
...
__name(latestLatents, "latestLatents");
...
__name(latentsByName, "latentsByName");
...
__name(boundsFrom, "boundsFrom");

// src/sources/metar.ts
var MetarCloudLayerSchema = external_exports.object({
  cover: external_exports.string(),
  base: external_exports.number().optional()
});
var MetarObservationSchema = external_exports.object({
  icaoId: external_exports.string(),
  receiptTime: external_exports.string(),
  obsTime: external_exports.number(),
  reportTime: external_exports.string(),
  temp: external_exports.number(),
  dewp: external_exports.number(),
  wdir: external_exports.number(),
  wspd: external_exports.number(),
  wg: external_exports.number().optional(),
  visib: external_exports.union([external_exports.number(), external_exports.string()]),
  altim: external_exports.number(),
  slp: external_exports.number().optional(),
  qcField: external_exports.number(),
  metarType: external_exports.string(),
  rawOb: external_exports.string(),
  lat: external_exports.number(),
  lon: external_exports.number(),
  elev: external_exports.number(),
  name: external_exports.string(),
  cover: external_exports.string(),
  clouds: external_exports.array(MetarCloudLayerSchema),
  wxString: external_exports.string().optional(),
  fltCat: external_exports.string()
});
var MetarResponseSchema = external_exports.array(MetarObservationSchema);
...
__name(bboxFromPoint, "bboxFromPoint");
var metarSource = {
  name: "METAR",
  cadenceMs: 5 * 60 * 1e3,
...
// src/sources/airnow.ts
var AirNowCategorySchema = external_exports.object({
  Number: external_exports.number(),
  Name: external_exports.string()
});
var AirNowObservationSchema = external_exports.object({
  DateObserved: external_exports.string(),
  HourObserved: external_exports.number(),
  LocalTimeZone: external_exports.string(),
  ReportingArea: external_exports.string(),
  StateCode: external_exports.string(),
  Latitude: external_exports.number(),
  Longitude: external_exports.number(),
  ParameterName: external_exports.string(),
  AQI: external_exports.number(),
  Category: AirNowCategorySchema
});
var AirNowResponseSchema = external_exports.array(AirNowObservationSchema);
function epochSeconds(o) {
  const d = /* @__PURE__ */ new Date(`${o.DateObserved}T${String(o.HourObserved).padStart(2, "0")}:00:00`);
...
// src/sources/firms.ts
var FirmsObservationSchema = external_exports.object({
  latitude: external_exports.number(),
  longitude: external_exports.number(),
  bright_ti4: external_exports.number().optional(),
  bright_ti5: external_exports.number().optional(),
  scan: external_exports.number().optional(),
  track: external_exports.number().optional(),
  acq_date: external_exports.string(),
  acq_time: external_exports.string(),
  satellite: external_exports.string().optional(),
  instrument: external_exports.string().optional(),
  confidence: external_exports.union([external_exports.string(), external_exports.number()]).optional(),
  version: external_exports.string().optional(),
  frp: external_exports.number().optional(),
  daynight: external_exports.string().optional()
});
var FirmsResponseSchema = external_exports.array(FirmsObservationSchema);
function bboxFromPoint2(lat, lon, radiusMiles) {
  const latDelta = radiusMiles / 69;
  const lonDelta = radiusMiles / (69 * Math.cos(lat * Math.PI / 180));
  return {
    south: lat - latDelta,
...
__name(bboxFromPoint2, "bboxFromPoint");
function epochSeconds2(o) {
  const hh = o.acq_time.padStart(4, "0").slice(0, 2);
  const mm = o.acq_time.padStart(4, "0").slice(2, 4);
  const d = /* @__PURE__ */ new Date(`${o.acq_date}T${hh}:${mm}:00Z`);
  return Math.floor(d.getTime() / 1e3);
}
__name(epochSeconds2, "epochSeconds");
...
var firmsSource = {
  ...
  async fetch(ctx) {
    ...
    const { south, west, north, east } = bboxFromPoint2(ctx.lat, ctx.lon, ctx.radiusMiles);
  ...
  observedAt(data) {
    ...
    return boundsFrom(data, epochSeconds2);
  }
};

// src/sources/hrrr_smoke.ts
var HrrrSmokeObservationSchema = external_exports.object({
  runTs: external_exports.number(),
  validTs: external_exports.number(),
  lat: external_exports.number(),
  lon: external_exports.number(),
  near_surface_smoke: external_exports.number()
});
var HrrrSmokeResponseSchema = external_exports.array(HrrrSmokeObservationSchema);
var hrrrSmokeSource = {
  name: "HRRR_SMOKE",
  cadenceMs: 60 * 60 * 1e3,
...
// src/sources/nexrad.ts
var NexradObservationSchema = external_exports.object({
  station: external_exports.string(),
  ts: external_exports.number(),
  vcp: external_exports.number().optional(),
  s3_key: external_exports.string()
});
var NexradResponseSchema = external_exports.array(NexradObservationSchema);
var nexradSource = {
  name: "NEXRAD",
  cadenceMs: 10 * 60 * 1e3,
  ...
  async fetch(ctx) {
    ...
    for (const station of stations) {
      ...
      results.push({
        station,
        ts: Math.floor(new Date(lastModMatch[1]).getTime() / 1e3),
        s3_key: keyMatch[1]
      });
    }
...
__name(latestPrefix, "latestPrefix");

// src/sources/nldn.ts
var NldnObservationSchema = external_exports.object({
  ts: external_exports.number(),
  lat: external_exports.number(),
  lon: external_exports.number(),
  peak_kA: external_exports.number(),
  type: external_exports.string()
});
var NldnResponseSchema = external_exports.array(NldnObservationSchema);
var nldnSource = {
  name: "NLDN",
  cadenceMs: 5 * 60 * 1e3,
...
// src/sources/events.ts
var EventObservationSchema = external_exports.object({
  ...
  venue_name: external_exports.string().optional(),
  category: external_exports.string().optional(),
  labels: external_exports.array(external_exports.string()).optional(),
  expected_presence: external_exports.number().optional(),
  rank: external_exports.number().optional(),
  local_rank: external_exports.number().optional(),
  confidence: external_exports.number(),
  url: external_exports.string().optional()
});
var EventResponseSchema = external_exports.array(EventObservationSchema);
var PredictHqEventSchema = external_exports.object({
  id: external_exports.string(),
  ...
  rank: external_exports.number().optional(),
  local_rank: external_exports.number().optional(),
  phq_attendance: external_exports.number().optional(),
  start: external_exports.string(),
  end: external_exports.string().optional(),
  predicted_end: external_exports.string().optional(),
  timezone: external_exports.string().optional(),
  location: external_exports.tuple([external_exports.number(), external_exports.number()]).optional(),
...
var PredictHqResponseSchema = external_exports.object({
  results: external_exports.array(PredictHqEventSchema)
});
var eventsSource = {
  name: "EVENTS",
  cadenceMs: 30 * 60 * 1e3,
  ...
  observedAt(data) {
    return boundsFrom(data, (o) => epochSeconds3(o.starts_at));
  }
};
async function fetchPredictHq(ctx) {
  const now = /* @__PURE__ */ new Date();
  const lookaheadHours = Number(ctx.env.EVENTS_LOOKAHEAD_HOURS ?? 24);
  const until = new Date(now.getTime() + lookaheadHours * 60 * 60 * 1e3);
...
__name(fetchPredictHq, "fetchPredictHq");
...
__name(toEventObservation, "toEventObservation");
function epochSeconds3(iso) {
  const t = Date.parse(iso);
  return Number.isNaN(t) ? 0 : Math.floor(t / 1e3);
}
__name(epochSeconds3, "epochSeconds");
...
var NOTAM_SOURCE_NAME = "NOTAM";
function sourceNames() {
  return registry.map((s) => s.name);
}
__name(sourceNames, "sourceNames");
function knownSourceNames() {
  return [...sourceNames(), NOTAM_SOURCE_NAME];
}
__name(knownSourceNames, "knownSourceNames");

// src/worker/collate.ts
async function collate(db, maxAgeMs, now = Date.now()) {
  ...
  for (const sourceName of knownSourceNames()) {
    ...
    if (!snap || snap.id === void 0) continue;
...
__name(collate, "collate");

// src/latents.ts
function deriveLatents(c, lookup, context2 = {}) {
  const out = [];
  const has = /* @__PURE__ */ __name((name) => Boolean(c.sources[name]), "has");
  const events = [...eventsFromSnapshot(has("EVENTS") ? lookup("EVENTS")?.data ?? [] : []), ...context2.events ?? []];
  const placeContext = buildPlaceContext(c, { ...context2, events });
  out.push({
  ...
  if (has("AIRNOW")) {
    ...
    if (snap) {
      const aqis = snap.data.map((d) => d.AQI).filter((n) => typeof n === "number");
      if (aqis.length > 0) {
  ...
  if (has("AIRNOW") && has("FIRMS")) {
    ...
    const fire2 = lookup("FIRMS");
    if (aq && fire2) {
      const pm = aq.data.filter((d) => /PM2\.5|PM10/.test(d.ParameterName ?? "")).map((d) => d.AQI).filter((n) => typeof n === "number");
      const pmMax = pm.length > 0 ? Math.max(...pm) : 0;
      const fires = fire2.data.length;
      out.push({
  ...
  if (has("AIRNOW") && has("FIRMS") && has("HRRR_SMOKE")) {
    ...
    if (aq && fire2 && hrrr) {
      ...
      const fireCount = fire2.data.length;
...
__name(deriveLatents, "deriveLatents");
function numbers(rows, key) {
  return rows.map((d) => d[key]).map((v) => typeof v === "number" ? v : Number(v)).filter((n) => Number.isFinite(n));
}
...
function maxAirnow(rows, parameterPattern) {
  return maxOf(
    rows.filter((d) => parameterPattern.test(d.ParameterName ?? "")).map((d) => d.AQI).filter((n) => typeof n === "number")
  );
}
__name(maxAirnow, "maxAirnow");
function minVisibilitySm(rows) {
  const values = numbers(rows, "visib");
  return values.length ? Math.min(...values) : void 0;
}
__name(minVisibilitySm, "minVisibilitySm");
...
__name(airQualitySummary, "airQualitySummary");
function severityFromAqi(aqi) {
  return aqi > 100 ? "alert" : aqi > 50 ? "watch" : "ok";
}
__name(severityFromAqi, "severityFromAqi");
function scoreAqi(aqi) {
  return clamp(aqi / 150 * 100, 0, 100);
}
__name(scoreAqi, "scoreAqi");
function scoreVisibility(sm) {
  return clamp((10 - sm) / 8 * 100, 0, 100);
}
__name(scoreVisibility, "scoreVisibility");
function scoreSmoke(value) {
  return clamp(value / 50 * 100, 0, 100);
}
__name(scoreSmoke, "scoreSmoke");
function scoreCount(count3, high) {
  return clamp(count3 / high * 100, 0, 100);
}
__name(scoreCount, "scoreCount");
function scoreWind(kt) {
  return clamp(kt / 35 * 100, 0, 100);
}
__name(scoreWind, "scoreWind");
function confidenceFromPresence(parts) {
  return Math.round(parts.filter(Boolean).length / parts.length * 100) / 100;
}
__name(confidenceFromPresence, "confidenceFromPresence");
function buildPlaceContext(c, context2) {
  const now = context2.now ?? c.collated_at;
  const timezone = context2.timezone ?? "America/Chicago";
  const locationLabel = context2.locationLabel ?? "Kansas City area";
  const local = localDateParts(now, timezone);
  const holidayName = usHolidayName(local.year, local.month, local.day);
  const weekend = local.weekday === "Sat" || local.weekday === "Sun";
  const activeEvents = (context2.events ?? []).filter((e) => now >= e.starts_at && now <= e.ends_at);
...
__name(buildPlaceContext, "buildPlaceContext");
...
__name(eventsFromSnapshot, "eventsFromSnapshot");
...
__name(temporalRhythmSummary, "temporalRhythmSummary");
...
__name(usHolidayName, "usHolidayName");
...
__name(nthWeekdayOfMonth, "nthWeekdayOfMonth");
...
__name(lastWeekdayOfMonth, "lastWeekdayOfMonth");
...
__name(activeNotams, "activeNotams");
...
__name(notamTextWeight, "notamTextWeight");
...
__name(summarizeNotamClasses, "summarizeNotamClasses");
...
// src/notam-schema.ts
var NotamRecordSchema = external_exports.object({
  id: external_exports.string(),
  fns_uuid: external_exports.string(),
  number: external_exports.string(),
  year: external_exports.string(),
  type: external_exports.string(),
  interpretation: external_exports.string(),
  issued: external_exports.string(),
  effective_start: external_exports.string(),
  effective_end: external_exports.string().optional(),
  location: external_exports.string(),
  icao_location: external_exports.string().optional(),
  airport_name: external_exports.string().optional(),
  affected_fir: external_exports.string().optional(),
  selection_code: external_exports.string().optional(),
  qline: external_exports.string().optional(),
  min_fl: external_exports.number().optional(),
  max_fl: external_exports.number().optional(),
  coordinates: external_exports.string().optional(),
  lat: external_exports.number().optional(),
  lon: external_exports.number().optional(),
  radius_nm: external_exports.number().optional(),
  classification: external_exports.string().optional(),
  text: external_exports.string(),
  local_format: external_exports.string().optional(),
  icao_format: external_exports.string().optional()
});
var NotamIngestRequestSchema = external_exports.object({
  records: external_exports.array(NotamRecordSchema)
});
function epochSeconds4(iso) {
  const t = Date.parse(iso);
  if (Number.isNaN(t)) return 0;
  return Math.floor(t / 1e3);
}
__name(epochSeconds4, "epochSeconds");
      ...
      body.innerHTML = \`<div class="latent-grid">\${sorted.map((l) => {
        ...
        return \`<article class="latent-card \${esc(severity)}">
          <div class="latent-top">
            <div class="latent-title"><strong>\${esc(l.label || l.name)}</strong><span>\${esc(l.family || "field")} \xB7 order \${esc(l.order || 1)}</span></div>
...
__name(renderFrontendHtml, "renderFrontendHtml");
...
function parseFieldLocation(params, defaults = KC_DEFAULTS) {
...
__name(parseFieldLocation, "parseFieldLocation");
function buildFieldContext(location, env2) {
  return {
    lat: location.lat,
    lon: location.lon,
...
__name(buildFieldContext, "buildFieldContext");
async function buildTransientField(location, env2, sources = registry, now = Date.now()) {
  const ctx = buildFieldContext(location, env2);
  const sourceResults = [];
  const snapshots = {};
  await Promise.all(
  ...
  const latents = collation ? deriveLatents(
    {
...
__name(buildTransientField, "buildTransientField");
...
__name(deriveLocationLabel, "deriveLocationLabel");
...
function summarizeMetar(data) {
  return data.slice(0, 8).map((row) => {
    const o = row;
    const tempF = typeof o.temp === "number" ? Math.round(o.temp * 9 / 5 + 32) : void 0;
    const dewF = typeof o.dewp === "number" ? Math.round(o.dewp * 9 / 5 + 32) : void 0;
    const wind = typeof o.wspd === "number" ? `${o.wspd} kt${typeof o.wdir === "number" ? ` from ${o.wdir}\xB0` : ""}` : "wind unavailable";
    const visibility = o.visib !== void 0 ? `${o.visib} sm visibility` : "visibility unavailable";
    return {
      source: "METAR",
      title: `${o.name ?? o.icaoId ?? "Station"}${o.icaoId ? ` (${o.icaoId})` : ""}`,
      summary: `${tempF ?? "-"}\xB0F, dew point ${dewF ?? "-"}\xB0F, ${wind}, ${visibility}, flight category ${o.fltCat ?? "unknown"}.`,
      severity: o.fltCat && !["VFR"].includes(o.fltCat) ? "watch" : "ok",
      details: { raw: o.rawOb }
    };
  });
}
__name(summarizeMetar, "summarizeMetar");
...
__name(summarizeAirNow, "summarizeAirNow");
function summarizeFirms(data) {
  const frps = data.map((d) => d.frp).filter((n) => typeof n === "number");
  const maxFrp = frps.length ? Math.max(...frps) : void 0;
  return [
    {
      source: "FIRMS",
      title: "Active fire detections",
      summary: `${data.length} detection(s) in the selected radius${maxFrp !== void 0 ? `; max FRP ${maxFrp}` : ""}.`,
      severity: data.length > 0 ? "watch" : "ok"
    }
  ];
}
__name(summarizeFirms, "summarizeFirms");
function summarizeHrrrSmoke(data) {
  const values = data.map((d) => d.near_surface_smoke).filter((n) => typeof n === "number");
  const max = values.length ? Math.max(...values) : void 0;
  return [
    {
      source: "HRRR_SMOKE",
      title: "Near-surface smoke",
      summary: `${data.length} model point(s)${max !== void 0 ? `; max near-surface smoke ${max}` : ""}.`,
      severity: max !== void 0 && max > 0 ? "watch" : "ok"
    }
  ];
}
__name(summarizeHrrrSmoke, "summarizeHrrrSmoke");
function summarizeNexrad(data) {
  return data.map((row) => {
    ...
    return {
      ...
      summary: `Latest scan metadata at ${o.ts ? new Date(o.ts * 1e3).toISOString() : "unknown time"}.`,
      details: { key: o.s3_key }
    };
  });
}
__name(summarizeNexrad, "summarizeNexrad");
function summarizeNldn(data) {
  const peaks = data.map((d) => Math.abs(d.peak_kA ?? 0));
  const maxPeak = peaks.length ? Math.max(...peaks) : void 0;
  return [
    {
      source: "NLDN",
      title: "Lightning detections",
      summary: `${data.length} strike(s)${maxPeak !== void 0 ? `; max peak ${maxPeak} kA` : ""}.`,
      severity: data.length > 0 ? "alert" : "ok"
    }
  ];
}
__name(summarizeNldn, "summarizeNldn");
function summarizeEvents(data) {
  return data.slice(0, 8).map((row) => {
    const o = row;
    const starts = o.starts_at ? new Date(o.starts_at).toLocaleString() : "unknown time";
    const presence = typeof o.expected_presence === "number" ? `; expected presence ${Math.round(o.expected_presence)}` : typeof o.local_rank === "number" ? `; local rank ${o.local_rank}` : "";
    return {
      source: "EVENTS",
      title: `${o.name ?? "Event"}${o.venue_name ? ` at ${o.venue_name}` : ""}`,
      summary: `${o.provider ?? "Event source"} ${o.category ?? "event"} starting ${starts}${presence}.`,
      severity: typeof o.expected_presence === "number" && o.expected_presence >= 5e3 ? "watch" : "ok"
    };
  });
}
__name(summarizeEvents, "summarizeEvents");
...
__name(endpointIndex, "endpointIndex");
function buildContext(env2) {
  return {
    lat: Number(env2.USER_LAT ?? 39.0997),
    lon: Number(env2.USER_LON ?? -94.5786),
    radiusMiles: Number(env2.RADIUS_MILES ?? 30),
    env: env2
  };
}
...
__name(runSource, "runSource");
async function runTick(env2) {
  const ctx = buildContext(env2);
  const now = Date.now();
  const reports = await Promise.all(registry.map((src) => runSource(env2.DB, src, ctx, now)));
  for (const r of reports) console.log(r);
  const maxAgeMs = Number(env2.COLLATION_MAX_AGE_MS ?? 60 * 60 * 1e3);
  const c = await collate(env2.DB, maxAgeMs, now);
  if (!c || c.id === void 0) return;
  const lookups = await Promise.all(
    Object.keys(c.sources).map(async (name) => [name, await latestSnapshot(env2.DB, name)])
  );
  const snapshotCache = new Map(lookups);
  const latents = deriveLatents(c, (name) => snapshotCache.get(name) ?? null);
  for (const l of latents) {
    await appendLatent(env2.DB, {
      ts: now,
      name: l.name,
      value: l.value,
      collation_id: c.id,
...
async function handleNotamIngest(req, env2) {
  if (!env2.INGEST_TOKEN) return json({ error: "ingest_disabled" }, 503);
  const auth = req.headers.get("authorization") ?? "";
  if (auth !== `Bearer ${env2.INGEST_TOKEN}`) return json({ error: "unauthorized" }, 401);
  let payload;
  try {
  ...
  const issuedTimes = records.map((r) => epochSeconds4(r.issued)).filter((t) => t > 0);
  const now = Date.now();
  const observedMin = issuedTimes.length > 0 ? Math.min(...issuedTimes) : Math.floor(now / 1e3);
  const observedMax = issuedTimes.length > 0 ? Math.max(...issuedTimes) : Math.floor(now / 1e3);
  await appendSnapshot(env2.DB, {
...
__name(handleNotamIngest, "handleNotamIngest");
var index_default = {
  async fetch(req, env2) {
    const url = new URL(req.url);
    if (req.method === "POST" && url.pathname === "/ingest/notam") {
      return handleNotamIngest(req, env2);
    }
    ...
    if (url.pathname === "/field/current") {
      try {
        const location = parseFieldLocation(url.searchParams, {
          ...KC_DEFAULTS,
          lat: Number(env2.USER_LAT ?? KC_DEFAULTS.lat),
          lon: Number(env2.USER_LON ?? KC_DEFAULTS.lon),
          radiusMiles: Number(env2.RADIUS_MILES ?? KC_DEFAULTS.radiusMiles)
        });
        return json(await buildTransientField(location, env2));
      } catch (err) {
        return json({ error: err instanceof Error ? err.message : "invalid_location" }, 400);
      }
    }
    if (url.pathname === "/collations/latest") {
      const c = await latestCollation(env2.DB);
      return c ? json(c) : json({ error: "no_collation" }, 404);
    }
    if (url.pathname === "/latents/latest") {
      const rows = await latestLatents(env2.DB);
